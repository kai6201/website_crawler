...
title_main <- web.text %>% html_nodes(".ui_header") %>% html_text()
...
for( page in 1:page_limit)
{
  ...
  for( i in 1:length(rs.crawler_more.list))
  {
    ...
    web.text <- rs.crawler_text$getElementAttribute("outerHTML")[[1]] %>% read_html()
    #各欄位爬文
    title <- web.text %>% html_nodes(".noQuotes") %>% html_text()#標題
    content <- web.text %>% html_nodes(".partial_entry") %>% html_text()#內容
    author <- web.text %>% html_nodes("div.info_text > div:nth-child(1)") %>% html_text()#作者
    country <- web.text %>% html_nodes("div.info_text > div:nth-child(2)") %>% html_text()#國籍
    date <- web.text %>% html_nodes(".ratingDate") %>% html_text()#日期
    date <- gsub("日的評論",replacement = "",date)
    date <- gsub("年|月",replacement = "/",date)
    url <- web.text %>% html_nodes("div.ui_column.is-9 > div.quote a") %>% html_attr("href")#網址
    url <- paste0("https://www.tripadvisor.com.tw",url)
    for (each in 1:length(score_class))#評價分數
    {
